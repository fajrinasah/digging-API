...
export const executeSelectPublishedArticlesFromUser = ({
  ...
  if (!filterByCategory && !filterByHeadline && !filterByKeywords) {
    generateExecuteFunction({
      queryStatements:
        "SELECT * FROM vw_articles_all WHERE username = ? ORDER BY article_id ? LIMIT 45",
  ...
  if (!filterByCategory && !filterByHeadline) {
    generateExecuteFunction({
      queryStatements:
        "SELECT * FROM vw_articles_all WHERE username = ? AND keywords LIKE ? ORDER BY article_id ? LIMIT 45",
  ...
  if (!filterByCategory && !filterByKeywords) {
    generateExecuteFunction({
      queryStatements:
        "SELECT * FROM vw_articles_all WHERE username = ? AND headline LIKE ? ORDER BY article_id ? LIMIT 45",
  ...
  if (!filterByHeadline) {
    generateExecuteFunction({
      queryStatements:
        "SELECT * FROM vw_articles_all WHERE username = ? AND category_id = ? AND keywords LIKE ? ORDER BY article_id ? LIMIT 45",
      placeholders: [
        username,
        filterByCategory,
        `%${filterByKeywords}%`,
  ...
  if (!filterByKeywords) {
    generateExecuteFunction({
      queryStatements:
        "SELECT * FROM vw_articles_all WHERE username = ? AND category_id = ? AND headline LIKE ? ORDER BY article_id ? LIMIT 45",
      placeholders: [
        username,
        filterByCategory,
        `%${filterByHeadline}%`,
